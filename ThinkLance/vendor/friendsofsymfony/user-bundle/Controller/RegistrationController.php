...
class RegistrationController extends Controller
{
    ...
    {
        //Pour éviter les champs duplicata
        $freelancers = $this->getDoctrine()->getRepository(\AppBundle\Entity\User::class)->findAll();
        foreach ($freelancers as $free) {
            if ($free->getEmail() == $request->get('email') || $free->getUsername() == $request->get('username') || $free->getFirstName() == $request->get('firstname')) {
                return new JsonResponse('Ce compte existe deja');
...
//        //Vérifier si l'utilisateur est connecté et le rediriger vers la page d'accueil si il l'est
...
//            return new RedirectResponse($router->generate('auth_homepage'), 307);
//        }
//        //End
...
//        //Si le Fomulaire est envoyé et valide
//        if ($form->isSubmitted()) {
//            if ($form->isValid()) {
//
