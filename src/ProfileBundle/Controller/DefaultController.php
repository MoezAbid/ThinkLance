...
class DefaultController extends Controller
{
    ...
    public function modifInfoUserAction($id,Request $request){
        $em = $this->getDoctrine()->getManager();
        ...
        $user1->setFirstName($request->get('firstname'));
        $encoded = $encoder->encodePassword($user1, $request->get('password'));
        $user1->setPassword($encoded);
        $em->flush();
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($user1);
        return new JsonResponse($formatted);

    }
public function addCommentaireAction($id,Request $request){
    $em = $this->getDoctrine()->getManager();
    $user1 = $em->getRepository(User::class)->find($id);
    ...
    $comment->setIdEmp($request->get('idemp'));
    $em->persist($comment);
    $em->flush();
    $serializer = new Serializer([new ObjectNormalizer()]);
...
public function getAllcommentsByIdAction($id){

    $competences = $this->getDoctrine()->getRepository(Commentaire::class)->getAllComments($id);
    $serializer = new Serializer([new ObjectNormalizer()]);
    $formatted = $serializer->normalize($competences);
    return new JsonResponse($formatted);
    ...
    public function  getAllcompetenceByIdAction($id){

        $competences = $this->getDoctrine()->getRepository(Competence::class)->getAllCompetences($id);
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($competences);
        return new JsonResponse($formatted);

    }
    public function noterUserAction($id,Request $request){
        $em = $this->getDoctrine()->getManager();
        $user1 = $em->getRepository(User::class)->find($id);
        $user1->setNote($user1->getNote()+$request->get('note')/2);
        $user1->setNbrPoints($request->get('nbPoints')+ $user1->getNbrPoints());
        $em->flush();
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($user1);
        return new JsonResponse($formatted);
...
public function findexperiencebyuserAction($id){
    $experiencesPro = $this->getDoctrine()->getRepository(ExperiencePro::class)->findExperiences($id);
    $serializer = new Serializer([new ObjectNormalizer()]);
    $formatted = $serializer->normalize($experiencesPro);
    return new JsonResponse($formatted);


}
    public function findformationbyuserAction($id){
        $formations = $this->getDoctrine()->getRepository(Formation::class)->findFormations($id);
        $serializer = new Serializer([new ObjectNormalizer()]);
...
//visualiser les informations de mon profil
    ...
    public function listerFreelancersAction()
    {
        ...
        $freelancers = $em->getRepository(User::class)->findAllFreelancers();

        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($freelancers);
        return new JsonResponse($formatted);
