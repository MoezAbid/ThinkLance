...
class TypeArticleController extends FOSController
{
    ...
    public function listerTypesArticlesApiAction(Request $request)
    {
        $typesArticles = $this->getDoctrine()->getRepository(TypeArticle::class)->findAll();
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($typesArticles);
        return new JsonResponse($formatted);
    }

    public function getIdTypeArticleSpecifiqueApiAction($nomTypeArticle, Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $article = $em->getRepository(TypeArticle::class)->trouverTypeArticleParNom($nomTypeArticle);
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($article);
        return new JsonResponse($formatted);
    }

    public function rechecherTypeArticleParIdApiAction($idCle, Request $request)
    {
        $em = $this->getDoctrine()->getManager();
        $article = $em->getRepository(TypeArticle::class)->find($idCle);
        $serializer = new Serializer([new ObjectNormalizer()]);
        $formatted = $serializer->normalize($article);
        return new JsonResponse($formatted);
    }
